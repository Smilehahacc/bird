...
import shutil
import sys
local_root = os.getcwd()+ '/blog/src/algorithm/SVM_KNN_BP/'
...
def Svm():
    ...
    # 训练集标签的位置
    train_label_path = os.path.join(local_root+'image','train.txt')
    # 测试集标签的位置
    test_label_path = os.path.join(local_root+'image128','train.txt')

    image_height = 128
    image_width = 100

    train_feat_path = local_root+'train/'
    test_feat_path = local_root+'test/'
    ...
    def train_and_test():
        ...
        print('准确率是： %f' % rate)
        print('耗时是 : %f' % (t1 - t0))
        return float(correct_number) / total
